...
const StoryPlayer: React.FC<StoryPlayerProps> = ({ media, storyData, onClose, customLogoUrl }) => {
  ...
  useEffect(() => {
    ...
    const isVideoMode = !!currentScene.videoUrl;
    ...
    // If image mode, start loop when image loads. If video mode, start immediately.
    if (isVideoMode) {
        animId = requestAnimationFrame(draw);
    } else {
        if (img.complete) animId = requestAnimationFrame(draw);
        else img.onload = () => { animId = requestAnimationFrame(draw); };
    }
