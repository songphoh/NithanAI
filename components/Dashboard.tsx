                ...
                {history.map((item) => (
                    <div key={item.storyData.id} className="group bg-slate-900 border border-slate-800 hover:border-indigo-500/50 rounded-xl overflow-hidden transition-all hover:shadow-xl hover:shadow-indigo-500/10 flex flex-col">
                        ...
                        <div className="relative aspect-[9/16] overflow-hidden">
                            <img 
                                src={item.storyData.coverImageUrl || item.media[0].imageUrl} 
                            ...
                            {/* Cover Title Overlay */}
                            <div className="absolute top-1/3 left-0 right-0 p-4 text-center transform -rotate-2">
                                <h3 className="text-3xl font-black text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] text-shadow-black leading-tight tracking-tighter uppercase break-words">
                                    {item.storyData.coverTitle || item.storyData.title}
                                </h3>
                            </div>
                        ...
                        {/* Content */}
                        <div className="p-4 flex-1 flex flex-col bg-slate-800/50">
                            <div className="flex-1">
                                <h4 className="text-white font-bold line-clamp-1 mb-1">{item.storyData.title}</h4>
                                <p className="text-slate-400 text-xs line-clamp-2 mb-3">{item.storyData.seoSummary}</p>
                            </div>
