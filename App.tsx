...
import { Sparkles, BookOpen, Loader2, Upload, X, LayoutDashboard, Plus, Menu, StopCircle, Mic, Film, Clock, Music, User, Settings2, Subtitles, Languages, Key, Image as ImageIcon } from 'lucide-react';
import { AppState, StoryData, GeneratedSceneMedia, HistoryItem, StoryMode, StoryConfig, VoiceGender, VoiceTone, SubtitleLang, MediaType } from './types';
import { generateStoryScript, generateLongStoryScript, generateSceneImage, generateSceneAudio, mapVoiceConfig, generateSceneVideo } from './services/geminiService';
...
function App() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      // 2. Check LocalStorage (for Vercel/Public)
    ...
    try {
      ...
      for (let i = 0; i < storyData.scenes.length; i++) {
        ...
        if (mediaType === 'video') {
            ...
            visualPromise = generateSceneImage(scene.imagePrompt);
        }
        
        const [visualResult, base64Audio] = await Promise.all([
          visualPromise,
        ...
        mediaItems.push({
          imageUrl: mediaType === 'image' ? visualResult : undefined,
          videoUrl: mediaType === 'video' ? visualResult : undefined,
      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4 font-['Prompt']">
        <div className="max-w-md w-full bg-slate-800/50 backdrop-blur-xl border border-slate-700 rounded-2xl p-8 text-center shadow-2xl animate-fade-in-up">
          ...
          <p className="text-slate-400 mb-8">
            Create magical Thai stories with AI narration and cinematic visuals (now with Veo Video!).
          </p>
          ...
          <div className="space-y-4">
            {(window as any).aistudio ? (
                ...
                >
                <Key size={18} /> Select Google Cloud API Key
                </button>
            ) : (
                <form onSubmit={handleManualKeySubmit} className="space-y-3">
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 text-left">
                         <p className="text-yellow-200 text-xs">
                             Enter your Gemini API Key. (Saved locally in browser).
                         </p>
                    </div>
                    <input 
                        type="password" 
                    ...
                    />
                    <button type="submit" className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl transition">
                        Start App
                    </button>
                </form>
            )}
    ...
    <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 border-r border-slate-800 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
             <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                ...
                <button onClick={() => { localStorage.removeItem("gemini_api_key"); window.location.reload(); }} className="text-xs text-slate-500 hover:text-red-400 mt-2 underline">
                    Clear API Key
                </button>
             </div>
        </div>
    ...
    <div className="w-full max-w-xl mx-auto flex flex-col items-center justify-center min-h-[calc(100vh-80px)] p-6">
         {appState === AppState.IDLE && (
            <div className="w-full flex flex-col gap-6 animate-fade-in-up">
                ...
                <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700 p-6 rounded-2xl shadow-xl space-y-5">
                    ...
                    {/* Media Type Selection (New Feature) */}
                    <div className="bg-slate-700/30 p-1 rounded-xl flex">
                        <button 
                            onClick={() => setMediaType('image')}
                            className={`flex-1 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition ${mediaType === 'image' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            <ImageIcon size={18} /> ภาพนิ่ง (Image)
                        </button>
                        <button 
                             onClick={() => setMediaType('video')}
                             className={`flex-1 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition ${mediaType === 'video' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            <Film size={18} /> วิดีโอ (Video Mode)
                        </button>
                    </div>
                    ...
                    >
                        {mediaType === 'video' ? <Film size={20} /> : <Sparkles size={20} />}
                        {mediaType === 'video' ? 'สร้างนิทานวิดีโอ (Video)' : 'สร้างนิทาน (Image)'}
                    </button>
                    {mediaType === 'video' && <p className="text-[10px] text-center text-slate-500">Note: Video generation (Veo) takes significantly longer than images. Requires Billing.</p>}
                </div>
            </div>
        )}
        ...
        {(appState === AppState.GENERATING_SCRIPT || appState === AppState.GENERATING_MEDIA) && (
             <div className="w-full flex flex-col items-center gap-6 animate-fade-in">
                ...
                <div className="text-center space-y-2">
                    ...
                    <p className="text-slate-400 max-w-xs mx-auto animate-pulse whitespace-pre-line">{statusMessage}</p>
                </div>
        ...
        {appState === AppState.ERROR && (
             <div className="text-center animate-fade-in">
                ...
                <p className="text-slate-400 mb-6 px-4 max-w-md mx-auto">{statusMessage}</p>
                <button 
                    onClick={resetApp}
